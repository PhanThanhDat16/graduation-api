...
export const vnpayController = {
  ...
  createPayment: expressAsyncHandler(async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
      ...
      // Tạo payment URL
      const paymentUrl = createPaymentUrl(vnp_TxnRef, amount, orderInfo, clientIp, bankCode, locale, returnUrl)
  ...
  handleReturn: expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const transaction = await WalletTransaction.findOne({ paymentOrderId: vnp_TxnRef as string }).populate(
        'userId',
        'fullName email'
      )
      if (!transaction) {
  ...
  vnpayIpn: expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { vnp_TxnRef, vnp_TransactionNo, vnp_ResponseCode, vnp_PayDate, vnp_TransactionStatus } = req.query

      const isValid = verifySecureHash(req.query as any, vnpayConfig.vnp_HashSecret as string)
      ...
      try {
        ...
        await session.withTransaction(async () => {
          const transaction = await WalletTransaction.findOne({ paymentOrderId: vnp_TxnRef }, null, { session })
          ...
          if (success) {
            ...
            await Wallet.findByIdAndUpdate(transaction.walletId, { $inc: { balance: transaction.amount } }, { session })
          } else {
