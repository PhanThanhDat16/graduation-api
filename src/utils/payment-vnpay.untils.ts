...
/* ===================== HASH ===================== */
export const createVNPayHash = (params: Record<string, string | number>, hashSecret: string): string => {
  const sortedKeys = Object.keys(params).sort()

  const signData = sortedKeys
    .map((key) => {
      return key + '=' + encodeURIComponent(String(params[key])).replace(/%20/g, '+')
    })
    .join('&')

  console.log('🔐 SIGN DATA (HASH):', signData)

  return crypto.createHmac('sha512', hashSecret).update(signData, 'utf-8').digest('hex')
}
...
export const createPaymentUrl = (
  ...
  const secureHash = crypto.createHmac('sha512', vnpayConfig.vnp_HashSecret).update(signData, 'utf-8').digest('hex')
...
export const verifyReturnHash = (query: Record<string, any>): boolean => {
  ...
  const calculatedHash = createVNPayHash(vnp_Params, vnpayConfig.vnp_HashSecret)
...
export const getPaymentStatusFromResponse = (responseCode: string): ETransactionStatus => {
  if (responseCode === '00') return ETransactionStatus.COMPLETED
  if (['24', '51', '65', '75'].includes(responseCode)) return ETransactionStatus.FAILED
  return ETransactionStatus.PENDING
}

export function verifySecureHash(vnpParams: Record<string, any>, secretKey: string): boolean {
  ...
  const calculatedHash = crypto.createHmac('sha512', secretKey).update(signData, 'utf-8').digest('hex')
